...
## בדיקת שטח (2026-07-01) — הורדת ספר שאינו במדף
נבחר ספר אקראי **שלא נמצא במדף**: "התפכחות" מאת דן מרגלית (consumableId `1373088`).
- אומת שהספר לא במדף (המדף החזיר 13 ספרים, ה-id לא ביניהם, `owns=0`).
- audiobook-dl **המתוקן** זיהה ואימת אותו בלי מדף: `1373088` → bookId `2615107` → AId `2615109`
  (עם `--print-output` הוצג "Downloading התפכחות from storytel").

### ⚠️ סייג חשוב — הבאג `get_files` חוסם את הקובץ בפועל
עקיפת המדף **מזהה ומאמתת** ספרים מחוץ למדף, אבל להורדת ה-mp3 בפועל עדיין חלה
תקלת ה-`get_files` של audiobook-dl 0.7.3 (יוצא `exit 0` בלי אודיו לחלק מהספרים).
בספר הזה הכלי יצא בלי קובץ. הפתרון: **הורדה ישירה** דרך אותו endpoint עוקף +
`mp3streamRangeReq` + תיוג ffmpeg — כך הושלם קובץ תקין (1:46:54, MP3 mono 64kbps,
עטיפה מוטמעת, תגים מלאים).

הערה: `scripts/storytel_direct_download.py` בריפו זה עדיין דורש מדף (מושך `getBookShelf`).
כדי להוריד ישירות ספרים שאינם במדף יש לבסס אותו על `getBookInfoForContent.action`
(כמו בפאץ' כאן) במקום על המדף.
